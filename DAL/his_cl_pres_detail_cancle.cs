...
namespace HIS.DAL
{
	...
	public partial class his_cl_pres_detail_cancle
	{
		...
		public HIS.Model.his_cl_pres_detail_cancle GetModel(string ID,string CL_PRES_CODE,string CL_CODE)
		{
			...
			strSql.Append(" where ID='"+ID+"' and CL_PRES_CODE='"+CL_PRES_CODE+"' and CL_CODE='"+CL_CODE+"' " );
			HIS.Model.his_cl_pres_detail_cancle model=new HIS.Model.his_cl_pres_detail_cancle();
		...
		/// 得到一个对象实体
		/// </summary>
		public HIS.Model.his_cl_pres_detail_cancle DataRowToModel(DataRow row)
		{
			HIS.Model.his_cl_pres_detail_cancle model=new HIS.Model.his_cl_pres_detail_cancle();
			if (row != null)
			{
