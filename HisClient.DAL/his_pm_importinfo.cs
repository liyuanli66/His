...
namespace HisClient.DAL
{
	...
	public partial class his_pm_importinfo
	{
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(HisClient.Model.his_pm_importinfo model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		public HisClient.Model.his_pm_importinfo GetModel(string ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select IMPORT_CODE,ID,MEDINFO_CODE,MED_SPC,MED_UNIT,MED_AMOUNT,MED_PRICE,PURCHASE_PRICE,WHOLESALE_PRICE,VALIDITY_DATE,BATCHNO,MED_MADETIME from his_pm_importinfo ");
			strSql.Append(" where ID=@ID ");
			MySqlParameter[] parameters = {
					new MySqlParameter("@ID", MySqlDbType.VarChar,18)			};
			parameters[0].Value = ID;

			HisClient.Model.his_pm_importinfo model=new HisClient.Model.his_pm_importinfo();
		...
		/// 得到一个对象实体
		/// </summary>
		public HisClient.Model.his_pm_importinfo DataRowToModel(DataRow row)
		{
			HisClient.Model.his_pm_importinfo model=new HisClient.Model.his_pm_importinfo();
			if (row != null)
			{
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "his_pm_importinfo";
