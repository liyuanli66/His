...
{
	/// <summary>
	/// 数据访问类:his_hos_pres_detail_cancle
	/// </summary>
	public partial class his_hos_pres_detail_cancle
	{
		...
		public bool Exists(string ID,string HOS_PRES_CODE,string HIS_HOS_CODE)
		{
			...
			strSql.Append("select count(1) from his_hos_pres_detail_cancle");
			strSql.Append(" where ID=@ID and HOS_PRES_CODE=@HOS_PRES_CODE and HIS_HOS_CODE=@HIS_HOS_CODE ");
			...
			parameters[1].Value = HOS_PRES_CODE;
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(HisClient.Model.his_hos_pres_detail_cancle model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into his_hos_pres_detail_cancle(");
			strSql.Append("ID,STATUS,APPLY_DEPT_CODE,DOCTOR_CODE,OPERATE_DEPT_CODE,SUM_AMT,OPT_USER,OPT_DATE,OPT_TERM,OPT_ORGA,HOS_PRES_CODE,HIS_HOS_CODE)");
			...
			parameters[2].Value = model.APPLY_DEPT_CODE;
			parameters[3].Value = model.DOCTOR_CODE;
			parameters[4].Value = model.OPERATE_DEPT_CODE;
			parameters[5].Value = model.SUM_AMT;
			...
			parameters[10].Value = model.HOS_PRES_CODE;
			parameters[11].Value = model.HIS_HOS_CODE;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update his_hos_pres_detail_cancle set ");
		...
		public bool Delete(string ID,string HOS_PRES_CODE,string HIS_HOS_CODE)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from his_hos_pres_detail_cancle ");
			strSql.Append(" where ID=@ID and HOS_PRES_CODE=@HOS_PRES_CODE and HIS_HOS_CODE=@HIS_HOS_CODE ");
			MySqlParameter[] parameters = {
					new MySqlParameter("@ID", MySqlDbType.VarChar,18),
					new MySqlParameter("@HOS_PRES_CODE", MySqlDbType.VarChar,18),
		...
		public HisClient.Model.his_hos_pres_detail_cancle GetModel(string ID,string HOS_PRES_CODE,string HIS_HOS_CODE)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,STATUS,APPLY_DEPT_CODE,DOCTOR_CODE,OPERATE_DEPT_CODE,SUM_AMT,OPT_USER,OPT_DATE,OPT_TERM,OPT_ORGA,HOS_PRES_CODE,HIS_HOS_CODE from his_hos_pres_detail_cancle ");
			strSql.Append(" where ID=@ID and HOS_PRES_CODE=@HOS_PRES_CODE and HIS_HOS_CODE=@HIS_HOS_CODE ");
			MySqlParameter[] parameters = {
					new MySqlParameter("@ID", MySqlDbType.VarChar,18),
					new MySqlParameter("@HOS_PRES_CODE", MySqlDbType.VarChar,18),
			...
			HisClient.Model.his_hos_pres_detail_cancle model=new HisClient.Model.his_hos_pres_detail_cancle();
		...
		/// 得到一个对象实体
		/// </summary>
		public HisClient.Model.his_hos_pres_detail_cancle DataRowToModel(DataRow row)
		{
			HisClient.Model.his_hos_pres_detail_cancle model=new HisClient.Model.his_hos_pres_detail_cancle();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,STATUS,APPLY_DEPT_CODE,DOCTOR_CODE,OPERATE_DEPT_CODE,SUM_AMT,OPT_USER,OPT_DATE,OPT_TERM,OPT_ORGA,HOS_PRES_CODE,HIS_HOS_CODE ");
			strSql.Append(" FROM his_hos_pres_detail_cancle ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from his_hos_pres_detail_cancle T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "his_hos_pres_detail_cancle";
