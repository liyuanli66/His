...
namespace HisClient.DAL
{
	...
	public partial class his_hos_order_item
	{
		...
		public bool Exists(string ID,string HOS_PRES_CODE,string HIS_HOS_CODE)
		{
			...
			strSql.Append("select count(1) from his_hos_order_item");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(HisClient.Model.his_hos_order_item model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into his_hos_order_item(");
			strSql.Append("ID,HOS_PRES_CODE,HIS_HOS_CODE,ITEM_CODE,ITEM_NAME,COUNT,PRICE,ITEM_TYPE,SPEC,UNIT,SUM_AMT,STATUS,HERB_NUM,OPT_USER,OPT_DATE,OPT_TERM,OPT_ORGA)");
			...
			parameters[1].Value = model.HOS_PRES_CODE;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update his_hos_order_item set ");
			...
			parameters[15].Value = model.HOS_PRES_CODE;
		...
		public bool Delete(string ID,string HOS_PRES_CODE,string HIS_HOS_CODE)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from his_hos_order_item ");
			strSql.Append(" where ID=@ID and HOS_PRES_CODE=@HOS_PRES_CODE and HIS_HOS_CODE=@HIS_HOS_CODE ");
			MySqlParameter[] parameters = {
					new MySqlParameter("@ID", MySqlDbType.VarChar,18),
					new MySqlParameter("@HOS_PRES_CODE", MySqlDbType.VarChar,18),
		...
		public HisClient.Model.his_hos_order_item GetModel(string ID,string HOS_PRES_CODE,string HIS_HOS_CODE)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,HOS_PRES_CODE,HIS_HOS_CODE,ITEM_CODE,ITEM_NAME,COUNT,PRICE,ITEM_TYPE,SPEC,UNIT,SUM_AMT,STATUS,HERB_NUM,OPT_USER,OPT_DATE,OPT_TERM,OPT_ORGA from his_hos_order_item ");
			strSql.Append(" where ID=@ID and HOS_PRES_CODE=@HOS_PRES_CODE and HIS_HOS_CODE=@HIS_HOS_CODE ");
			MySqlParameter[] parameters = {
					new MySqlParameter("@ID", MySqlDbType.VarChar,18),
					new MySqlParameter("@HOS_PRES_CODE", MySqlDbType.VarChar,18),
			...
			HisClient.Model.his_hos_order_item model=new HisClient.Model.his_hos_order_item();
		...
		/// 得到一个对象实体
		/// </summary>
		public HisClient.Model.his_hos_order_item DataRowToModel(DataRow row)
		{
			HisClient.Model.his_hos_order_item model=new HisClient.Model.his_hos_order_item();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,HOS_PRES_CODE,HIS_HOS_CODE,ITEM_CODE,ITEM_NAME,COUNT,PRICE,ITEM_TYPE,SPEC,UNIT,SUM_AMT,STATUS,HERB_NUM,OPT_USER,OPT_DATE,OPT_TERM,OPT_ORGA ");
			strSql.Append(" FROM his_hos_order_item ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from his_hos_order_item T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "his_hos_order_item";
