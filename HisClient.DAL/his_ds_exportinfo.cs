...
namespace HisClient.DAL
{
	...
	public partial class his_ds_exportinfo
	{
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(HisClient.Model.his_ds_exportinfo model)
		{
			StringBuilder strSql=new StringBuilder();
			...
			MySqlParameter[] parameters = {
					...
					new MySqlParameter("@MED_NAME", MySqlDbType.VarChar,20),
			...
			parameters[3].Value = model.MED_CODE;
			parameters[4].Value = model.MED_NAME;
			parameters[5].Value = model.PAKAGE_UNIT;
			parameters[6].Value = model.PAKAGE_AMOUNT;
			parameters[7].Value = model.MED_PRICE;
			parameters[8].Value = model.PURCHASE_PRICE;
		...
		public HisClient.Model.his_ds_exportinfo GetModel(string ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,EXPORT_CODE,MEDINFO_CODE,MED_CODE,MED_NAME,PAKAGE_UNIT,PAKAGE_AMOUNT,MED_PRICE,PURCHASE_PRICE,VALIDITY_DATE,BATCHNO,MED_MADETIME from his_ds_exportinfo ");
			strSql.Append(" where ID=@ID ");
			MySqlParameter[] parameters = {
					new MySqlParameter("@ID", MySqlDbType.VarChar,18)			};
			parameters[0].Value = ID;

			HisClient.Model.his_ds_exportinfo model=new HisClient.Model.his_ds_exportinfo();
		...
		/// 得到一个对象实体
		/// </summary>
		public HisClient.Model.his_ds_exportinfo DataRowToModel(DataRow row)
		{
			HisClient.Model.his_ds_exportinfo model=new HisClient.Model.his_ds_exportinfo();
			if (row != null)
			{
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "his_ds_exportinfo";
