...
namespace HisClient.DAL
{
	...
	public partial class his_cl_prescription
	{
		...
		public bool Exists(string CL_PRES_CODE,string CL_CODE)
		{
			...
			parameters[0].Value = CL_PRES_CODE;
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(HisClient.Model.his_cl_prescription model)
		{
			StringBuilder strSql=new StringBuilder();
			...
			parameters[0].Value = model.CL_PRES_CODE;
		...
		public bool Update(HisClient.Model.his_cl_prescription model)
		{
			...
			parameters[9].Value = model.CL_PRES_CODE;
			parameters[10].Value = model.CL_CODE;
		...
		public HisClient.Model.his_cl_prescription GetModel(string CL_PRES_CODE,string CL_CODE)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select CL_PRES_CODE,CL_CODE,STATUS,APPLY_DEPT_CODE,DOCTOR_CODE,OPERATE_DEPT_CODE,SUM_AMT,OPT_USER,OPT_DATE,OPT_TERM,OPT_ORGA from his_cl_prescription ");
			strSql.Append(" where CL_PRES_CODE=@CL_PRES_CODE and CL_CODE=@CL_CODE ");
			MySqlParameter[] parameters = {
					new MySqlParameter("@CL_PRES_CODE", MySqlDbType.VarChar,18),
					new MySqlParameter("@CL_CODE", MySqlDbType.VarChar,18)			};
			...
			HisClient.Model.his_cl_prescription model=new HisClient.Model.his_cl_prescription();
		...
		/// 得到一个对象实体
		/// </summary>
		public HisClient.Model.his_cl_prescription DataRowToModel(DataRow row)
		{
			HisClient.Model.his_cl_prescription model=new HisClient.Model.his_cl_prescription();
			if (row != null)
			{
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "his_cl_prescription";
			parameters[1].Value = "CL_CODE";
