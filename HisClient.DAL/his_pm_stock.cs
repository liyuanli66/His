...
namespace HisClient.DAL
{
	...
	public partial class his_pm_stock
	{
		...
		public bool Exists(string ID)
		{
			...
			strSql.Append("select count(1) from his_pm_stock");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(HisClient.Model.his_pm_stock model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into his_pm_stock(");
			strSql.Append("ID,MEDINFO_CODE,MED_SPC,MED_UNIT,MED_AMOUNT,MED_PRICE,PURCHASE_PRICE,WHOLESALE_PRICE,VALIDITY_DATE,MED_MADETIME,BATCHNO,DEPT_CODE)");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update his_pm_stock set ");
			...
			parameters[7].Value = model.VALIDITY_DATE;
			parameters[8].Value = model.MED_MADETIME;
		...
		public bool Delete(string ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from his_pm_stock ");
			strSql.Append(" where ID=@ID ");
			MySqlParameter[] parameters = {
					new MySqlParameter("@ID", MySqlDbType.VarChar,18)			};
			parameters[0].Value = ID;
		...
		public HisClient.Model.his_pm_stock GetModel(string ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,MEDINFO_CODE,MED_SPC,MED_UNIT,MED_AMOUNT,MED_PRICE,PURCHASE_PRICE,WHOLESALE_PRICE,VALIDITY_DATE,MED_MADETIME,BATCHNO,DEPT_CODE from his_pm_stock ");
			strSql.Append(" where ID=@ID ");
			MySqlParameter[] parameters = {
					new MySqlParameter("@ID", MySqlDbType.VarChar,18)			};
			parameters[0].Value = ID;

			HisClient.Model.his_pm_stock model=new HisClient.Model.his_pm_stock();
		...
		/// 得到一个对象实体
		/// </summary>
		public HisClient.Model.his_pm_stock DataRowToModel(DataRow row)
		{
			HisClient.Model.his_pm_stock model=new HisClient.Model.his_pm_stock();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,MEDINFO_CODE,MED_SPC,MED_UNIT,MED_AMOUNT,MED_PRICE,PURCHASE_PRICE,WHOLESALE_PRICE,VALIDITY_DATE,MED_MADETIME,BATCHNO,DEPT_CODE ");
			strSql.Append(" FROM his_pm_stock ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from his_pm_stock T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "his_pm_stock";
